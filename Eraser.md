...
- 替换为面积擦
  - 颜色形状：白色圆形
  - 大小
    - `currentState == TouchState.Eraser` 且多指: 直径等于手指间最大间距
    - 其他情况：50px
    - 在进入新的触摸周期时更新大小
- 激活/隐藏时机
    - 注：隐藏时需同步隐藏白色圆形（避免干扰视线）、失去擦除功能（避免误擦除）
    - 工具栏未选中面积擦
      - 严格遵循现有状态机模型：`currentState == TouchState.Eraser`
    - 工具栏已选中面积擦
      - 严格遵循现有状态机模型：`currentState == TouchState.Eraser || currentState == TouchState.Draw || currentState == TouchState.EvalDraw || currentState == TouchState.MultiDraw`
